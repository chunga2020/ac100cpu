...
*** JUMP
- All jump instructions may take a raw address or a label (see JE examples)
- 0x30 :: Jump if equal (JE)
  ...
  - Example 2: JE done
    - Assuming ‘done’ is a valid previously-defined label, if the two values in
      the preceding comparison were equal, jump to the memory location
      associated with the label ‘done’
